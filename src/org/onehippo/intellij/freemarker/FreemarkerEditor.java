...
/**
 * @version "$Id: FreemarkerEditor.java 195460 2014-05-05 11:55:56Z mmilicevic $"
 */
public class FreemarkerEditor extends AnAction {

    public static final NotificationGroup ERROR_GROUP = new NotificationGroup("Hippo Freemarker editor error messages", NotificationDisplayType.BALLOON, true);
    public static final NotificationGroup INFO_GROUP = new NotificationGroup("Hippo Freemarker editor info messages", NotificationDisplayType.NONE, false);
    ...
    public void actionPerformed(final AnActionEvent event) {
        ...
        if (project != null) {
            ...
            String ftlFolder = canonicalFile.getPath() + File.separator + RepositoryConnector.FTL_FOLDER;
            if (component != null) {
            ...
            saveFile(connector, ftlFolder, currentFile, charsSequence.toString());

        }

    }

    public void saveFile(final RepositoryConnector connector, final String ftlFolder, final VirtualFile currentFile, final String content) {
        String path = currentFile.getPath();
        // check if file is within  freemarker root folder
        if (!path.startsWith(ftlFolder)) {
            error("File: " + path + " is not within Freemarker root folder", project);
            return;
        }

        path = path.substring(ftlFolder.length(), path.length());
